...
class DQN:
    ...
    @lp_print_every_iter
    @lp
    def stepSGD(self, goal, data: list, terminal_state: bool):

        for experience in data:
            showState_inline(experience.St, str='st')
            showState_inline(experience.Stplus1, str='stplus1')
            ...
            with torch.no_grad():
                if terminal_state:
                    ...
                    target = self.gamma * goal + Rtplus1
...
def showState_inline(state, str=''):
    ...
    cv2.imshow(str, cv2.pyrDown(canvas))
    cv2.waitKey(1)
    return None
...
try:
    ...
    while True:
        ...
        while not done:
            ...
            for i in range(6):
                Stplus1, Rtplus1_frame, done_frame, __ = env.step(At)
            ...
            if memory.enough_samples:
                # Update the network
                dqn.stepSGD(goal=Q(Stplus1,Atplus1,ww), data=minibatch, terminal_state=done)

            qstate = qstate_tplus1
